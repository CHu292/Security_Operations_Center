def scrambling(input_bits):
    ...
    # Quy ước ban đầu: các bit B trước khi tính toán (B_0, B_1, ..., B_4) là 0
    for i in range(n):
        # Lấy bit A_i từ chuỗi nhập vào
        A_i = int(input_bits[i])
        
        # Tính toán B_i theo công thức B_i = A_i XOR B_{i-3} XOR B_{i-5}
        B_i_minus_3 = B[i - 3] if i >= 3 else 0
        B_i_minus_5 = B[i - 5] if i >= 5 else 0
        
        # Tính giá trị của B_i
        B[i] = A_i ^ B_i_minus_3 ^ B_i_minus_5
