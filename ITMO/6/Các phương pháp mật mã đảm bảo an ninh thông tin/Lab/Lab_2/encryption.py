...
# Input: 64-bit plaintext (hex) and 64-bit key (hex)
...
def des_encrypt_block(plaintext_bits, key_bits):
    ...
    final_block = permute(r + l, IP_1)
    return final_block

def hex_to_bits(hex_string):
    return [int(b) for b in bin(int(hex_string, 16))[2:].zfill(len(hex_string)*4)]
...
key = "13345799BBCDFF11"
plaintext_bits = hex_to_bits(plaintext)
key_bits = hex_to_bits(key)
cipher_bits = des_encrypt_block(plaintext_bits, key_bits)
